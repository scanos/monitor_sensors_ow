...
outside_temp=$(echo $(sudo curl "https://api.openweathermap.org/data/2.5/weather?lat=54.38&lon=-5.54&APPID=YOURAPIcredentials" | jq .main.temp) - 273.15 | bc)
check_value "$outside_temp" "Outside" "temperature" "C"
#$wind_speed = $temp*2.23694;
wind=$( echo $(sudo curl "https://api.openweathermap.org/data/2.5/weather?lat=54.38&lon=-5.54&APPID=APPID=YOURAPIcredentials" | jq '.wind.speed'))
